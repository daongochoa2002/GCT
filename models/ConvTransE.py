...
class ConvTransE(torch.nn.Module):
    ...
    def forward(self, query_ent_embeds, query_rel_embeds, output, all_embeds):
        batch_size = query_ent_embeds.size(0)
        stacked_inputs = torch.cat((query_ent_embeds.unsqueeze(1), query_rel_embeds.unsqueeze(1), output.unsqueeze(1)), dim=1)
